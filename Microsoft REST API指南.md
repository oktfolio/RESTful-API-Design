...
本文档建立了Microsoft REST API应该遵循的指导原则，以便一致的开发RESTful接口。

## 2. 目录
 

## 3. 引言
开发人员通常通过HTTP接口访问大多数微软云平台资源。虽然每个服务通常提供特定于语言框架来包装其API，但它们的所有操作最终都归结为HTTP请求。微软必须支持广泛的客户端和服务，不能依赖于每个开发环境都有丰富的框架。因此，本指导原则的目标是确保Microsoft REST api能够被任何具有基本HTTP支持的客户机轻松且一致地使用。
[*]译者注：本指南不限于微软技术和平台，广泛适应于各种语言和平台。

为了给开发人员提供最流畅的体验，让这些API遵循一致的设计准则是很重要的，从而让使用它们变得简单和直观。本文档建立了 Microsoft REST API 开发人员应该遵循的指南, 以便统一一致地开发API。
...
- 让所有程序开发人员都可以轻松的通过REST接口访问Micosoft服务。
- 允许Service开发人员利用其他Service的基础上来开发一致的REST API端点。
...
- REST on Wikipedia — REST背后的常见定义和核心思想概述。
...
### 5.1 错误
错误，或者更具体地说是服务错误，定义为客户端向服务传递无效数据，并且服务正确拒绝该数据。示例包括无效凭证、错误的参数、未知的版本ID等。客户端传递错误的或者不合法的数据的情况通常返回 “4xx” 的 HTTP 错误代码。
...
故障会影响整体 API 的可用性。
由于速率限制或配额故障而失败的调用不能算作故障。由于服务快速失败(fast-failing)请求(通常是为了保护自己)而失败的调用会被视为故障。
...
### 5.5 长期运行API故障
对于长时间运行的 API，很可能出现第一次请求成功，且后续每次去获取结果时 API 也处于正常运行（每次都回传 200）中，但其底层操作已经失败了的情况。长时间运行故障必须作为故障汇总到总体可用性指标中。

## 6. 客户端指导
为确保客户端与REST服务对话时获得最佳体验，客户端应遵循以下最佳实践:

### 6.1 忽略规则
对于松散耦合的客户端调用，在调用之前不知道数据的确切定义和格式，如果服务器没用返回客户端预期的内容，客户端必须安全地忽略它。

有些服务（接口）可能在不更改版本号的情况下向响应添加字段。此类服务必须在其文档中明确这一点，客户必须忽略这些未知字段。
...
服务端也可以在协议中明确指定某些元素按特定方式进行排序。例如，服务端可以每次返回 JSON 对象时都把 JSON 对象的类型信息作为第一个字段返回，进而简化客户端解析返回数据格式的难度。客户端处理数据时可以依赖于服务端明确指定了的排序行为。
...
### 7.3 规范标识符
除了友好的URL之外，可以移动或重命名的资源应该公开包含唯一固定标识符的URL。
...
### 7.4 支持方法
操作必须尽可能使用正确的HTTP方法，并且必须尊重操作的幂等性。HTTP方法通常称为HTTP动词。
...
POST操作应该支持重定向响应标头（Location），以便通过重定向标头到任何未明确命名的已创建资源的位置。
...
调用者在create上指定键值（key）的服务应该支持UPSERT语义，该服务必须支持使用PATCH创建资源。
因为PUT被定义为内容的完全替换，所以客户端使用PUT修改数据是危险的。
当试图更新资源时，不理解(并因此忽略)资源的某些属性的客户端，很可能在PUT上忽视这些属性，导致提交后这些属性可能在不经意间被删除。
服务可以选择支持PUT来更新现有资源，但如果支持，则必须是完整替换(即，PUT之后，资源的属性必须匹配请求中提供的内容，包括删除没有提供的任何服务端的属性)。
...
OPTIONS 允许客户端检索关于资源的信息，并至少可以通过返回表示该资源的有效方法的Allow 标头来实现。
[*]译者注：当发起跨域请求时，浏览器会自动发起OPTIONS请求进行检查。

此外，服务应该包括一个链接头（Link header）(见RFC 5988)，指向有关资源的文档:
...
[*]译者注：例如分页时，返回下一步、上一步链接信息。这方面，大家可以参阅Github的API，如下所示：
...
下面的请求标头表 应该遵循 Microsoft REST API指南服务使用。使用这些标题不是强制性的，但如果使用它们则必须始终如一地使用。
...
## 7.8. 将标头指定为查询参数
有些标头对某些场景(如AJAX客户端)不兼容，特别是在不支持添加标头的跨域调用时。因此，除了常见的标头信息外，一些标头信息可以作为查询参数被接受，其命名与头信息相同:
...
## 7.9. PII 个人身份信息参数
与组织的隐私政策一致，客户端不应该在URL中传输个人身份信息(PII)参数(作为路径或查询字符串的一部分)，因为这些信息可能通过客户端、网络和服务器日志和其他机制无意暴露出来。

因此，服务应该接受PII参数作为标头传输。

然而，由于客户端或软件的限制，在许多情况下无法遵循上述建议。为了解决这些限制，服务也应该接受这些PII参数作为URL的一部分，与本指导原则的其余部分保持一致。

接受PII参数(无论是在URL中还是作为标头)的服务 应该符合其组织的领导层指定的隐私策略。通常建议包括：客户端使用标头进行传输，并且实现要遵循特殊的预防措施，以确保日志和其他服务数据收集得到正确的处理。

[*]译者注：PII——个人可标识信息。比如家庭地址，身份证信息。

## 7.10. Response formats 响应格式
一个成功的平台，必须以开发人员习惯使用的格式和一致的方式提供数据，并允许开发人员使用公共 Http 代码处理响应。

基于Web的通信，特别是当涉及移动端或其他低带宽客户端时，由于各种原因，已经迅速向JSON方向发展。主要是由于其更轻量以及易于与JavaScript交互。

JSON属性名应该采用CAMELCASE驼峰命名规范。

服务应该提供JSON作为默认输出格式。

### 7.10.1 Clients-specified 客户端指定响应格式

在HTTP中，客户端应该使用Accept头请求响应格式。 服务端可以选择性的忽略，即使这不是典型的良好的服务。客户端可以发送多个Accept标头，服务可以选择其中一个格式进行响应。
...
### 7.10.2 错误的条件响应
对于调用不成功的情况，开发人员应该能够编写一段代码，在不同的Microsoft REST API指导准则服务中一致地处理错误。这允许构建简单可靠的基础架构来处理异常，将异常作为成功响应的独立处理流程来处理。下面的代码基于OData v4 JSON规范。但是，它非常通用，不需要特定的OData构造。即使api没有使用其他OData结构，也应该使用这种格式。

错误响应必须是单个JSON对象。该对象必须有一个名为“error”的 名称/值（name/value） 对。该值必须是JSON对象。

这个对象必须包含名称“code”和“message”的 键值对，并且它可能包含名称“target”、“details”和 “innererror” 的键值对。

“code”键值对的值 是一个与语言无关的字符串。它的值是该服务定义的错误代码，应该简单可读。与响应中指定的HTTP错误代码相比，此代码用作错误的更具体的指示。服务应该具有相对较少的“代码”数量(大约20个)，并且所有客户端必须能够处理所有这些值。
...
在本例中，最基本的错误代码是“BadArgument”，但是对于感兴趣的客户端，“innererror”中有更具体的错误代码。
“passwordreusenotal”代码可能是在稍后时间由该服务添加的，之前只返回“passwordnotmeetpolicy”。
添加新错误代码时，现有客户机不会中断，但是新客户端可能会利用它。
...
应使用标准HTTP状态码; 更多信息，请参见HTTP状态代码定义。
...
- 对于POST调用，在适用的情况下，选择简单的内容类型(“application/x-www-form-urlencoded”、“multipart/form-data”、“text/plain”)。任何其他内容类型都会引发预检请求。
	- 服务不得以避免CORS预检请求的名义违反其他API建议。特别是，根据建议，由于内容类型的原因，大多数POST请求实际上需要预检请求。
	- 如果取消预检工作是关键，那么一项服务可能支持数据传输的其他机制，但也必须支持建议的方法。
...
## 9. 集合 Collections
### 9.1. Item keys
服务可以支持集合中每个项的持久标识符(主键)，该标识符应用JSON表示为"id" , 这些持久标识符通常用作项目的键。

支持持久标识符(主键)的集合可能支持增量查询。
...
随着数据的增长，集合也在增长。
预先计划采用分页对所有服务都很重要。
因此，当多个页面可用时，序列化有效负载(payload)必须适当地包含下一页的不透明URL。
有关详细信息，请参阅分页指南。
...
POST请求不是幂等的。
这意味着发送到具有完全相同的有效负载(payload)的集合资源的两个POST请求可能导致在该集合中创建多个项。
...
跨页面的排序参数必须一致，因为客户端和服务器端分页都完全兼容排序。
...
将类型与值分开的一个好处是，一旦客户端知道特定服务使用的类型，它就可以解释该值而无需任何额外的解析。
...
服务可以选择“主要”版本方案，在这种情况下暗示“.0”，本节中的所有其他规则适用。
...
3.即使通过API的未来版本，保证REST API的URL路径稳定性的服务也可以采用查询字符串参数机制。 这意味着API中描述的关系的命名和结构在API发布后无法发展，即使在具有重大更改的版本中也是如此。
4.无法确保未来版本的URL路径稳定性的服务必须将该版本嵌入到URL路径中。

某些基础服务（如Microsoft的Azure Active Directory）可能会暴露在多个端点之后。
这些服务必须支持每个端点的版本控制机制，即使这意味着支持多个版本控制机制。
...
一个示例可能是计算机重新启动，其中操作本身同步完成，但虚拟机资源上的GET操作将具有可以随时查询的“状态：重新启动”，“状态：正在运行”。
...
逐步操作需要很长时间，而且通常是不可预测的完成时间，并且不提供在资源中建模的状态转换。
...
本文档使用术语“逐步长时间运行的操作”，或者通常只是“逐步操作”，以避免混淆“异步”一词。

服务必须对逐步请求执行尽可能多的同步验证。
服务必须以同步方式对返回错误进行优先级排序，其目标是仅使用长时间运行的操作线协议处理“有效”操作。

对于定义为逐步长时间运行的操作的API，即使操作可以立即完成，服务也必须经过逐步长时间运行的操作流。
...
显然，面对来自调用方的潜在无限负载，服务无法保证这些响应时间。因此，服务应该为客户端设计和记录调用请求限制，并在超过这些限制时使用适当的、可操作的错误和错误消息进行响应。
当服务通常过载时，它们应该快速响应错误，而不是简单地缓慢响应。
...
服务应该返回 429 来处理客户端调用过多的情况，并且可以通过更改调用模式来修复这种情况。
如果一般负载或个别调用者无法控制的其他问题导致服务变慢，则服务应使用503进行响应。
在所有情况下，服务还应该提供信息，建议调用者在再次尝试之前应该等待多长时间。
客户端应该尊重这些报头，并实现其他瞬时故障处理技术。
...
2. 花费超过1秒的呼叫在第99百分位响应应该使用长时间运行模式
3. 持续时间超过0.5秒的呼叫在99％的响应中应该强烈考虑LRO模式
...
2. 调用者可以假设，在接收到带有Retry-After标头的响应之后，请求是重试的，而不需要对请求做任何更改。
